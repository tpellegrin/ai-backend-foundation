...
- **Composition transitive-reach clarification ([ADR-0025](../adr/0025-direct-import-semantics-for-core-wiring-only-infra.md)).** `app.main` transitively reaches `app.infrastructure.*` via `app.core.lifespan → app.core.wiring.<capability>`. This is the sanctioned ADR-0023 composition flow. The `core-wiring-only-infra` `import-linter` contract carries `allow_indirect_imports = True` and enforces the **direct-import ban only**: only `app.core.wiring.*` may write `from app.infrastructure... import ...`. The transitive dimension is enforced independently by the `Layers` contract. **Dynamic imports (`importlib.import_module("app.core.wiring.<x>")`), function-local imports of wiring from `lifespan.py`, and re-export shims that hide a composition edge from static analysis are forbidden.** T-701 owns the `.importlinter` flip in the same PR that materializes the first `app.core.wiring.* → app.infrastructure.*` edge; subsequent wiring tasks (T-702, T-703, T-704, T-708, T-1212, T-1402, T-1503, worker wiring, provider adapters) do **not** re-edit `.importlinter` on this axis and use static, top-of-file `from app.core.wiring.<x> import <symbol>` imports in `app.core.lifespan`.
...
- **Infrastructure is an outer adapter ring ([ADR-0026](../adr/0026-infrastructure-as-outer-adapter-ring.md)).** `app.infrastructure.*` is modeled as an **outer driven-adapter ring**, not a lower layer of the core. For Phase 2, this ring is for driven adapters only: database, Redis, storage, queues, HTTP clients, provider SDKs, vector stores, rate limiting, and idempotency stores called by the application. Entrypoints and delivery mechanisms such as `app.main`, `app.api`, and future `app.worker` are not part of `app.infrastructure`; do not place API handlers, workers, CLI commands, migrations, or scheduler code there merely because they interact with the outside world. Adapters may import inward to the ports they implement — `app.platform.*.ports`, `app.llm.ports`, `app.embeddings.ports`, `app.prompts.ports`, `app.rag.ports`, `app.ai.ports`, `app.ai_governance.ports`, `app.auth.ports`, `app.users.ports`, `app.documents.ports`. Non-wiring modules must **not** import `app.infrastructure.*` directly: `app.core.wiring.*` is the sole approved runtime binding surface (ADR-0023, ADR-0025). This is enforced mechanically by explicit infrastructure boundary contracts (`app.infrastructure` is outside the `Layers` contract). Infrastructure adapters must not import `app.main`, `app.api`, or `app.core` (they implement ports, not call back into application entrypoints). Adapters should not import other adapters; cross-adapter composition belongs in `app.core.wiring.*`. No dynamic imports, local imports, re-export shims, or `ignore_imports` entries may be used to hide infrastructure edges from static analysis. Consequences for implementers: (a) an adapter task must **not** edit `.importlinter` for its adapter-to-port import — the edge is already allowed; if `lint-imports` fails on that direction, **stop and report**; (b) do not use `importlib.import_module(...)`, function-local imports, or re-export shims to hide an adapter edge from static analysis; (c) do not add `ignore_imports` entries to whitelist an architecturally-correct edge — fix the contract shape via an ADR instead; (d) adapters must **structurally** satisfy their Protocol ports and must not inherit from them unless a task or ADR explicitly requires nominal inheritance (see `patterns.md` — Ports and adapters: structural conformance).
